...
4. **Reuse existing agents.** Implementation uses `do-develop`, `do-frontend`, `do-reviewer`, `code-architect`, `spec-explorer` — only planning and testing use spec-specific agents.
...
8. **No complexity-based shortcuts in Phase 2.** Phase 2 steps (spec-explorer → spec-planner → write plan.md) are MANDATORY for ALL tasks, regardless of perceived simplicity. You may NOT skip `codeagent-wrapper --agent spec-explorer` or `codeagent-wrapper --agent spec-planner` even for "simple", "single-file", or "obvious" tasks. Only Phase 1 has skip conditions — Phase 2 does not. If a task seems too simple for `/spec`, the user should use `/do` or `/omo` instead; once `/spec` is invoked, the full Phase 2 pipeline applies.
...
> **NO SHORTCUTS — THIS PHASE IS NEVER SKIPPABLE.** Every step below (spec-explorer → spec-planner → write plan.md) is mandatory for ALL tasks, no matter how simple or obvious. You must NOT generate plan.md content yourself or skip the spec-explorer/spec-planner agents. If you catch yourself thinking "this is simple enough to handle directly" — STOP. That reasoning violates Hard Constraint #8. The only phase with skip conditions is Phase 1.
