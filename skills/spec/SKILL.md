...
8. **No complexity-based shortcuts in Phase 2.** Phase 2 steps (explore → spec-planner → write plan.md) are MANDATORY for ALL tasks, regardless of perceived simplicity. You may NOT skip `codeagent-wrapper --agent explore` or `codeagent-wrapper --agent spec-planner` even for "simple", "single-file", or "obvious" tasks. Only Phase 1 has skip conditions — Phase 2 does not. If a task seems too simple for `/spec`, the user should use `/do` or `/omo` instead; once `/spec` is invoked, the full Phase 2 pipeline applies.
...
- **Skipping explore**: Reasoning that codebase exploration is unnecessary for a small change. The `explore` agent provides context that improves plan quality even for trivial tasks. This violates constraints #6 and #8.
- **Using built-in Agent/Explore**: Substituting Claude Code's built-in Explore subagent for `codeagent-wrapper --agent explore`. These are different systems. This violates constraint #6.
...
> **NO SHORTCUTS — THIS PHASE IS NEVER SKIPPABLE.** Every step below (explore → spec-planner → write plan.md) is mandatory for ALL tasks, no matter how simple or obvious. You must NOT generate plan.md content yourself or skip the explore/spec-planner agents. If you catch yourself thinking "this is simple enough to handle directly" — STOP. That reasoning violates Hard Constraint #8. The only phase with skip conditions is Phase 1.
...
### Step 2: Explore codebase (MANDATORY: via codeagent-wrapper)

> **IMPORTANT**: You MUST use `codeagent-wrapper --agent explore` via the Bash tool below. Do NOT use Claude Code's built-in Explore subagent or Agent tool — they are different systems. If Bash requires user approval, request it; do not fall back to built-in tools.
