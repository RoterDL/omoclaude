...
7. **Plan.md content must come from `spec-planner` agent.** The orchestrator writes the output file, but the content MUST be generated by `codeagent-wrapper --agent spec-planner`. Do NOT generate plan.md content directly — delegate content generation to the agent and write its output.
